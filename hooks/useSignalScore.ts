...
export function useSignalScore(ticker: string = '') {
  ...
  const vixTimerRef   = useRef<NodeJS.Timeout | null>(null)
  const macroTimerRef = useRef<NodeJS.Timeout | null>(null)
  ...
  useEffect(() => {
    ...
    fetchMacro()
    fetchPpp()
    vixTimerRef.current   = setInterval(fetchVix,   5  * 60 * 1000)
    macroTimerRef.current = setInterval(fetchMacro, 60 * 60 * 1000)
    return () => {
      if (vixTimerRef.current)   clearInterval(vixTimerRef.current)
      if (macroTimerRef.current) clearInterval(macroTimerRef.current)
    }
